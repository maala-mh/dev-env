...
GCLOUD_APP_CRED_PATH="$HOME/.config/gcloud/application_default_credentials.json"
platform=`uname`
...
echo "${yel}Installing pulumi..${yel}${end}"
...
echo "${yel}when I added this, I was not sure what command I used to install google cloud sdk${yel}${end}"
# this is essential for apps i.e, pulumi to use the cred file generated by this command to auth to gcloud
gcloud auth application-default login

# 2. Check if the file physically exists
if [ ! -f "$GCLOUD_APP_CRED_PATH" ]; then
    echo "❌ Error: Credential file not found at $GCLOUD_APP_CRED_PATH"
